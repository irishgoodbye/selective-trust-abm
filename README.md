...
Credibility isn't a property of a signal. It's a judgment an audience makes by checking a new act against everything it already knows about the actor. That judgment can be withdrawn.
## What's in here
...
Every deviation from the group norm gets scored on consistency, visibility, and cultural similarity. These multiply together, so no single factor can carry a deviation on its own
Prestige builds only through credited deviation. Conformity holds your position. It never grows it
A collapse mechanism (reappraisal) can wipe an agent's prestige and history in a single tick. This models how a reputation can fail all at once rather than erode gradually
...
The collapse mechanism, at its original settings, only ever caught agents with no history at all. It never caught the established actors the theory is actually about. Two implementation bugs were found and fixed along the way
Once recalibrated, the mechanism turned out to be highly selective. It reaches only the one agent tier whose cultural traits are randomly assigned rather than clustered, which is exactly the population most likely to end up genuinely out of step with its neighbors
